...
bookbuddy_normalised AS (
    ...
    FROM bookbuddy_keyed AS bb
    LEFT JOIN author_name_mapping AS anm
...
isbn_matches AS (
    SELECT
        ...
        'isbn' AS match_type
    FROM bookbuddy_normalised AS bb
    INNER JOIN goodreads_normalised AS gr
        ON
            bb.isbn IS NOT NULL
...
title_author_matches AS (
    ...
    INNER JOIN goodreads_normalised AS gr
        ON
            bb.title_key = gr.title_key
            AND bb.author_key = gr.author_key
    WHERE
        bb.book_id NOT IN (SELECT bb_book_id FROM isbn_matches)
...
matched AS (
    ...
    FROM all_matches AS m
    INNER JOIN bookbuddy_normalised AS bb ON m.bb_book_id = bb.book_id
    INNER JOIN goodreads_normalised AS gr ON m.gr_book_id = gr.book_id
    LEFT JOIN genre_mapping AS gm ON lower(trim(bb.genre)) = gm.raw_genre_key
    LEFT JOIN author_countries AS ac ON bb.author_key = ac.author_key
    LEFT JOIN country_name_fr AS cnf ON lower(trim(ac.country)) = cnf.country_key
    LEFT JOIN manual_ratings AS mr
        ON
            bb.title_key = mr.title_key
...
goodreads_only AS (
    SELECT
        ...
        cast(NULL AS STRING) AS match_type,
        coalesce(cnf.country_fr, ac.country) AS country
    FROM goodreads_normalised AS gr
    LEFT JOIN author_countries AS ac ON gr.author_key = ac.author_key
    LEFT JOIN country_name_fr AS cnf ON lower(trim(ac.country)) = cnf.country_key
    LEFT JOIN manual_ratings AS mr
        ON
            gr.title_key = mr.title_key
